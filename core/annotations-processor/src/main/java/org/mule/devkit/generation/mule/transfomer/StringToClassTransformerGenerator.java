...
public class StringToClassTransformerGenerator extends AbstractMessageGenerator {
    ...
    protected void doGenerate(DevKitTypeElement typeElement) {
        ...
        context.note("Generating String to Class transformer as " + transformerClass.fullName());
    ...
    private void generateDoTransform(DefinedClass definedClass) {
        ...
        tryStatement.body()._return(
                ref(Class.class).staticInvoke("forName").arg(ExpressionFactory.cast(ref(String.class), src))
        );
        CatchBlock catchBlock = tryStatement._catch(ref(ClassNotFoundException.class));
        ...
        Invocation errorMessage = ref(MessageFactory.class).staticInvoke("createStaticMessage").
                arg(ref(String.class).staticInvoke("format").arg("Could not parse %s as a class").arg(src));
    ...
    private void generateConstructor(DefinedClass transformerClass) {
        ...
        registerDestinationType(constructor, ref(Class.class));
    ...
    private DefinedClass getTransformerClass(DevKitTypeElement typeElement) {
        String transformerClassName = context.getNameUtils().generateClassNameInPackage(typeElement, NamingContants.STRING_TO_CLASS_TRANSFORMER_CLASS_NAME);
