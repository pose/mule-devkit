...
public class NestedsBuilder extends BaseStudioXmlBuilder {
            ...
            if (needToCreateNestedElement(variable)) {
                ...
                if (isSimpleList(variable)) {
                    ...
                    if (isListOfMaps(variable)) {
                        ...
                        NestedElementReference childElement1 = createChildElement(variable, SchemaConstants.INNER_PREFIX + NameUtils.singularize(localId));
    ...
    private void handleSimpleMap(Variable parameter, NestedElementType secondLevelNestedElement) {
        ...
        attributeTypeForMapKey.setName(SchemaConstants.ATTRIBUTE_NAME_KEY);
...
//        }
    ...
    private NestedElementType createSecondLevelNestedElement(Variable parameter, NestedElementReference childElement) {
        ...
        if (isListOfMaps(parameter)) {
            nestedElement.setLocalId(SchemaConstants.INNER_PREFIX + NameUtils.singularize(localIdSuffix));
            nestedElement.setXmlname(SchemaConstants.INNER_PREFIX + NameUtils.uncamel(NameUtils.singularize(parameter.getSimpleName().toString())));
        } else {
